...
library(leaps)
dim(TrainATransform)
#Look at BIC graph: minimum bic at 11 variables
set.seed(1234)
...
coef(reg.fwd,11)
final.model = lm(loglabels~log_pct_civilian_labor+log_pct_uninsured_adults+
                   air_pollution_particulate_matter+pct_physical_inacticity+
                   pct_adult_obesity+death_rate_per_1k+pct_adults_with_high_school_diploma+
                   pct_american_indian_or_alaskan_native+pct_hispanic+pct_aged_65_years_and_older+
                   economic_typology, data=TrainATransform)

vif(final_model)[,3]^2 #looks good
summary(final_model) #drop air_pollution_particulate_matter, pct_hispanic

final.model = lm(loglabels~log_pct_civilian_labor+log_pct_uninsured_adults
                   +pct_physical_inacticity+
                   pct_adult_obesity+death_rate_per_1k+pct_adults_with_high_school_diploma+
                   pct_american_indian_or_alaskan_native+pct_aged_65_years_and_older+
                   economic_typology, data=TrainATransform)

vif(final.model)[,3]^2 #looks good
summary(final.model) #all significant
...
#Predict on test set
test = read.csv('/Users/spencerfogelman/Desktop/Stats2/Project/microsoft-data-science-capstone/Test_values.csv')
test = test[, -1]
head(test)

test_yearA = test[test$yr == 'a',]
head(test_yearA)

test_yearA = subset(test_yearA, select = -(yr))

names(test_yearA) = sub("area__|econ__|demo__|health__", "", names(test_yearA))
names(test_yearA)

test_yearA = select(test_yearA, -c('homicides_per_100k', 'pct_excessive_drinking'))
...
'Nonmetro - Urban population of 20,000 or more, not adjacent to a metro area' = 9
"
test_yearA$rucc = car::recode(test_yearA$rucc, recoded)
...
'Noncore adjacent to micro area and contains a town of 2,500-19,999 residents' = 12
"
test_yearA$urban_influence = car::recode(test_yearA$urban_influence, recoded2)

test_yearA$rucc_recoded = NA


test_yearA$rucc_recoded[test_yearA$rucc %in% c(1,2,6)] = 'Metro'
test_yearA$rucc_recoded[!(test_yearA$rucc %in% c(1,2,6))] = 'Nonmetro'

test_yearA[, c('rucc', 'rucc_recoded')]

TestATransform = data.frame(test_yearA)
TestATransform$loglabels = log(testyearA$train_labels)
TestATransform$log_pop_per_primary_care_physician = log(test_yearA$pop_per_primary_care_physician)
TestATransform$log_pop_per_dentist = log(test_yearA$pop_per_dentist)
TestATransform$log_motor_vehicle_crash_deaths_per_100k = log(test_yearA$motor_vehicle_crash_deaths_per_100k)
TestATransform$log_pct_low_birthweight = log(test_yearA$pct_low_birthweight)
TestATransform$log_pct_adults_bachelors_or_higher = log(test_yearA$pct_adults_bachelors_or_higher)
TestATransform$log_pct_adults_less_than_a_high_school_diploma = log(test_yearA$pct_adults_less_than_a_high_school_diploma)
TestATransform$log_pct_asian = log(test_yearA$pct_asian + 0.01) #Need to offset because contains 0
TestATransform$log_pct_below_18_years_of_age = log(test_yearA$pct_below_18_years_of_age)
TestATransform$log_pct_uninsured_adults = log(test_yearA$pct_uninsured_adults)
TestATransform$log_pct_unemployment = log(test_yearA$pct_unemployment)
TestATransform$log_pct_civilian_labor = log(test_yearA$pct_civilian_labor)

labels_to_drop = names(TestATransform) %in% c('train_labels', 'pop_per_primary_care_physician', 'pop_per_dentist', 'motor_vehicle_crash_deaths_per_100k',
                                               'pct_low_birthweight', 'pct_adults_bachelors_or_higher', 'pct_adults_less_than_a_high_school_diploma',
                                               'pct_asian', 'pct_below_18_years_of_age', 'pct_uninsured_adults', 'pct_unemployment',
                                               'pct_civilian_labor')

TestATransform = TestATransform[, !labels_to_drop]

to_drop = names(TestATransform) %in% c('urban_influence', 'rucc_recoded')
TestATransform = TestATransform[, !to_drop]

test_predictions = predict(final.model, newdata=TestATransform)
