...
first_model_transform = lm(loglabels ~ ., data=TrainATransform, singular.ok = TRUE)
vif(first_model_transform) #All vifs good
...
qqnorm(lm_subset$residuals)
qqline(lm_subset$residuals)
...
index<-which(bics==min(bics)) #18 variables
points(index,bics[index],col="red",pch=10)

#50/50 Train and test split
set.seed(1234)
index=sample(1:dim(subset_train)[1],1599,replace=F)
train=TrainATransform[index,]
test = TrainATransform[-index,]

#Rerun forward selection
reg.fwd=regsubsets(loglabels~.,data=TrainATransform,method="forward",nvmax=18)
